...
vector<double> mid_head_vec[THREADS_NUM][2];
vector<double> mid_tail_vec[THREADS_NUM][2];
vector<double> mid_grad[THREADS_NUM];
vector<double> mid_norm_vec[THREADS_NUM];
vector<double> mid_norm_grad[THREADS_NUM];
...
void run(int n_in,double rate_in,double margin_in,int method_in)
{
	...
	for(int i=0; i<THREADS_NUM; i++)
	{
		...
		mid_head_vec[i][0].resize(n);
		mid_head_vec[i][1].resize(n);
		mid_tail_vec[i][0].resize(n);
		mid_tail_vec[i][1].resize(n);
		mid_grad[i].resize(n);
		mid_norm_vec[i].resize(n);
		mid_norm_grad[i].resize(n);
	}
...
void calc_entity_vec(int head, int tail, int rel, int flag, int tid)		//use Recursive Hierarchy Encoder
{
...
{
	double sum=0;
	calc_entity_vec(e1, e2, rel, flag, tid);
...
void gradient_triple(int e1_a,int e2_a,int rel_a,int e1_b,int e2_b,int rel_b,int tid)		//SGD update
{
	...
	for(int i=0; i<n; i++)
	{
		...
		for(int ii=0; ii<n; ii++)
		{
			type_mat_tmp[tempType][i][ii] += der_1 * mid_head_vec[tid][1][ii];
	...
	for(int i=0; i<n; i++)
	{
		for(int ii=0; ii<n; ii++)
		{
			domain_mat_tmp[tempDomain][i][ii] += mid_grad[tid][i] * entity_vec[e1_a][ii];
			entity_tmp[e1_a][ii] += mid_grad[tid][i] * domain_mat[tempDomain][i][ii];
		}
	}
	//tail
	...
	for(int i=0; i<n; i++)
	{
		...
		for(int ii=0; ii<n; ii++)
		{
			type_mat_tmp[tempType][i][ii] += der_1 * mid_tail_vec[tid][1][ii];
			mid_grad[tid][ii] += der_1 * type_mat[tempType][i][ii];
		}
	}
	for(int i=0; i<n; i++)
	{
		for(int ii=0; ii<n; ii++)
		{
			domain_mat_tmp[tempDomain][i][ii] += mid_grad[tid][i] * entity_vec[e2_a][ii];
			entity_tmp[e2_a][ii] += mid_grad[tid][i] * domain_mat[tempDomain][i][ii];
		}
	}
	//negative triple
	for (int i = 0;i<n;i++)
	...
	for(int i=0; i<n; i++)
	{
		...
		for(int ii=0; ii<n; ii++)
		{
			type_mat_tmp[tempType][i][ii] += der_1 * mid_head_vec[tid][0][ii];
			mid_grad[tid][ii] += der_1 * type_mat[tempType][i][ii];
		}
	}
	for(int i=0; i<n; i++)
	{
		for(int ii=0; ii<n; ii++)
		{
			domain_mat_tmp[tempDomain][i][ii] += mid_grad[tid][i] * entity_vec[e1_b][ii];
			entity_tmp[e1_b][ii] += mid_grad[tid][i] * domain_mat[tempDomain][i][ii];
		}
	}
	//tail
	...
	for(int i=0; i<n; i++)
	{
		...
		for(int ii=0; ii<n; ii++)
		{
			type_mat_tmp[tempType][i][ii] += der_1 * mid_tail_vec[tid][0][ii];
			mid_grad[tid][ii] += der_1 * type_mat[tempType][i][ii];
		}
	}
	for(int i=0; i<n; i++)
	{
		for(int ii=0; ii<n; ii++)
		{
			domain_mat_tmp[tempDomain][i][ii] += mid_grad[tid][i] * entity_vec[e2_b][ii];
			entity_tmp[e2_b][ii] += mid_grad[tid][i] * domain_mat[tempDomain][i][ii];
		}
	}
}
