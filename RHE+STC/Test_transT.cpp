...
public:
    ...
    {
		//relation_vec
        FILE* f1 = fopen(("../res_RHE/relation2vec."+version).c_str(),"r");
		...
		//entity_vec
		FILE* f2 = fopen(("../res_RHE/entity2vec."+version).c_str(),"r");
		...
		//type_mat
		FILE* f3 = fopen(("../res_RHE/typeMatrix."+version).c_str(),"r");
		...
		//domain_mat
		FILE* f4 = fopen(("../res_RHE/domainMatrix."+version).c_str(),"r");
		...
		mid_head_vec.resize(n);
		mid_tail_vec.resize(n);
		...
		for(int ent=0; ent<entity_num; ent++)
		{
			...
			for(int rel=0; rel<relation_num; rel++)
			{
				...
				for(int i=0; i<n; i++)
				{
					...
					for(int ii=0; ii<n; ii++)
					{
						mid_head_vec[i] += domain_mat[tempHeadDomain][i][ii] * entity_vec[ent][ii];
						mid_tail_vec[i] += domain_mat[tempTailDomain][i][ii] * entity_vec[ent][ii];
					}
				}
				for(int i=0; i<n; i++)
				{
					head_entity_vec[ent][rel][i] = 0;
					tail_entity_vec[ent][rel][i] = 0;
					for(int ii=0; ii<n; ii++)
					{
						head_entity_vec[ent][rel][i] += type_mat[tempHeadType][i][ii] * mid_head_vec[ii];
						tail_entity_vec[ent][rel][i] += type_mat[tempTailType][i][ii] * mid_tail_vec[ii];
					}
				}
			}
			if(ent % 100 == 0)
